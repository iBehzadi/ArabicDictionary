...
<template >
  <div>
    <header2 @back="router.back()">
      <template v-slot:title>{{CategoryDB.categoryTitle[+route.params.id]}}</template>
    </header2>
    <!-- route.params.data -->
    <!-- <WordComponent class="mt-1" :obj="obj" v-for="item in 10" :key="i"></WordComponent> -->
    ...
    <button @click="router.push('/Practice')" class="bg-yellow w-full h-11 rounded-t-2xl flex justify-center pt-3 fixed bottom-0">
      ...
      <p class="text-sm text-center pr-2">تمرین لغات {{CategoryDB.categoryTitle[+route.params.id]}} </p>
    </button>
  </div>
</template>
