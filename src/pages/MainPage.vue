...
<template>
  <!-- SETTING MENU-->
  <div ref="settings" @click="closeSetting"
    class="bg-gray-300 t-0 right-0 left-0 bottom-0 fixed z-20 h-full w-0 bg-opacity-50">
    <div ref="settingSide" id="setting-nav"
      class="h-full w-0 t-0 right-0 overflow-x-hidden bg-white bg-opacity-100 transition-all duration-300" @click.stop>
      <div class="flex flex-col mt-5">
        <button aria-label="Paid version" @click="paidVersionModal" class="text-yellow">
          خرید نسخه طلایی
        </button>
        <button aria-label="Help" @click="guideModal">راهنما</button>
        <button aria-label="About Us" @click="router.push('/About')">
          درباره ما
        </button>
        <button aria-label="Install app" v-if="showInstallButton" @click="install">
          نصب برنامه
        </button>
      </div>
    </div>
  ...
  <div>
    <header class="sticky left-0 right-0 top-0 z-10 lg:hidden md:hidden">
      <div class="pb-1 pl-1 border-b bg-white flex items-center justify-between">
        ...
        <div class="flex items-center justify-between mt-1 lg:mr-3" v-if="mobileView">
          <button aria-label="Settings menu" @click="openSetting" class="text-gray-darkest mr-4 flex-center">
        ...
        <!-- title -->
        <div class="h-full">
          <span class="inline-block text-center sm:text-3xl md:text-4xl text-black font-bold leading-4 mt-2">دیکشرنی عربی
            نبراس</span>
      ...
      <div class="pt-1 relative bg-gray-100">
        <form action="#" class="flex mr-2 ml-2">
          <input @input.stop="SearchCall()" type="text" v-model="search"
            class="text-1xl h-14 rounded-full w-full bg-white pr-6 pl-12 outline-none" placeholder="جستجو کنید..." />
          <font-awesome-icon :icon="['fas', 'search']" class="absolute text-lg text-gray-400 left-7 top-5" />
        </form>
      </div>
    </header>
    ...
    <header class="w-full shadow-shadowBottom z-50 top-0 sticky hidden lg:flex md:flex">
      <div class="h-full w-full">
        <div class="h-20 bg-gray-50">
          <div class="float-right text-4xl font-bold text-gray-600 leading-4 items-center mt-1 h-3/4 pr-11 pt-5">
            دیکشرنی عربی نبراس
            <div class="w-full text-center mb-2 text-gray-400 font-quran leading-none">
              <span class="text-2xs sm:text-base">
                لهجه
        ...
        <div class="w-full text-center pr-4 flex bg-gray-200 h-11">
          <ul class="flex blackk tracking-tighter w-auto leading-9 text-base cursor-pointer">
            <li @click="paidVersionModal" class="mr-2 pl-3 h-8 hover:text-blue-400 inline-block">
              خرید نسخه طلایی
            </li>
            <li @click="guideModal" class="mr-4 pl-3 h-8 hover:text-blue-400">
              راهنما
            </li>
            <li @click="router.push('/About')" class="mr-4 pl-3 h-8 hover:text-blue-400">
              درباره ما
            </li>
            <li class="dropdown">
              <div
              ...
              <div
                ...
                <button @click="goToVocabularyTest()" class="hover:bg-blue-100 p-3">
                  تمرین لغات
                </button>
          ...
          <div
            ...
            <input @input.stop="SearchCall()" v-model="search"
              ...
              type="text" placeholder="جستجوکنید..." />
            <a class="text-gray-700 w-10 h-10 float-left bg-blue-100 rounded-4xl flex justify-center items-center relative bottom-1 left-0"
              href="#">
    ...
    <div class="bg-gray-100 z-2 lg:mt-1 md:mt-1 xl:mt-1">
      <!--category -->
      <div class="flex flex-fill justify-content-center flex-wrap text-center pb-16 md:pb-28 justify-evenly">
          ...
          <div
            ...
            <router-link :to="{ name: 'bookmark' }">
              <div class="h-full flex-center flex-col md:flex-row md:justify-evenly">
                <div class="sm:w-12 md:w-24 pt-4 md:pt-0 lg:pt-0 xl:pt-0 md:absolute md:right-0 category_icon">
                  <font-awesome-icon :icon="['fas', 'bookmark']" class="text-green-600 md:w-2" />
                </div>
                <span class="text-black text-xs left-3 sm:text-sm md:text-lg lg:text-1xl xl:text-1xl">نشان شده ها</span>
              </div>
            </router-link>
          </div>
          <!-- Popular suggestions -->
          <div
            ...
            <router-link :to="{ name: 'popular' }">
              <div class="h-full flex-center flex-col md:flex-row md:justify-evenly">
                ...
                <span class="text-black text-xs left-3 sm:text-sm md:text-lg lg:text-1xl xl:text-1xl">پیشنهادات
                  مردمی</span>
              </div>
            </router-link>
          </div>

          <div v-for="(item, i) in categoryRepo.category" :key="i"
            ...
            <a class="cursor-pointer" @click="openCategory(item.IsFree, i)">
              <div class="h-full flex-center flex-col md:flex-row md:justify-evenly">
                <font-awesome-icon v-if="!item.IsFree" :icon="['fas', 'lock']"
                  class="absolute text-gray-500 left-3 top-3 text-sm sm:text-base md:text-2xl xl:text-3xl" />
                <div class="md:absolute md:right-1 md:w-24 pt-4 md:pt-0 lg:pt-0 xl:pt-0 flex-center category_icon"
                  v-html="categoryRepo.category[i].Icon"></div>
                <span :class="{ 'text-gray-500': !item.IsFree }"
                  class="text-black text-xs sm:text-sm md:text-lg lg:text-1xl xl:text-1xl">{{ item.Title }}</span>
              </div>
            </a>
          </div>
        </template>
        <TranslateRequest @request="wordTranslateRequest" v-if="wordRepo.searchResult.length === 0 && isNotFoundSearch">
        </TranslateRequest>
      ...
      <div
        ...
        <button class="w-2/4 sm:w-3/5 md:w-2/6 border rounded-t-2xl bg-yellow text-center" @click="goToVocabularyTest()">
          <span class="text-sm sm:text-base md:text-xl lg:text-2xl xl:text-3xl">
            <font-awesome-icon :icon="['fas', 'pen']"
              class="text-sm text-gray-600 ml-2 sm:text-base md:text-xl lg:text-2xl xl:text-3xl" />تمرین لغات
          </span>
        </button>
        <button class="w-2/4 sm:w-3/5 md:w-2/6 border rounded-t-2xl bg-yellow text-center mr-1">
          <span class="text-sm sm:text-base md:text-xl lg:text-2xl xl:text-3xl">
            <font-awesome-icon :icon="['fas', 'question']"
              class="text-sm text-gray-600 ml-2 sm:text-base md:text-xl lg:text-2xl xl:text-3xl" />آزمون مرحله ای
          </span>
        </button>
      </div>
    </div>
    ...
    <div v-if="needRefresh && offSWModal"
      class="fixed flex-center top-0 left-0 right-0 bottom-0 bg-gray-400 bg-opacity-50 z-50 text-center" role="alert">
      <div class="bg-uniqColor rounded-md p-1">
        <div class="mb-4 p-3">
          <span v-if="needRefresh">نسخه جدید برنامه در دسترس است. مایل به بروزرسانی هستید؟</span>
        </div>
        <button class="outline-none mr-2 rounded-md bg-green-500 mx-2 p-3 hover:bg-blue-400" v-if="needRefresh"
          @click="updateAndCloseSWModal">
          بروزرسانی
        </button>
        <button class="outline-none mr-2 rounded-md bg-green-500 mx-2 p-3 hover:bg-blue-400" @click="closeSWUpdateModal">
          فعلا نه
        </button>
      </div>
    </div>
